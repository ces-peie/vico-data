...
raw_lab_respira_resultados <- load_raw_data("RespiratorioResultados.csv")


#---- Merge data sources ----
respira_hospital <- raw_sujeto_hospital %>%
  ...
  left_join(raw_lab_respira_resultados, by="SubjectID")


#---- Keep enrolled cases ----
respira_hospital <- respira_hospital %>%
  filter(!is.na(SASubjectID) & elegibleRespira == 1)
  

#---- Unload pracessed data block ----
unload_data_block(respira_hospital)
